...
namespace BarcodePcApp
{
    public partial class FormPrintBarcodes : Form
    {
        ...
        public FormPrintBarcodes(int orgID, int lokalID, int cabinetID, int orgAr, string nodeDB)
        {
            ...
            if (lang ==true)
            {
                this.printButton.Text = "Skriv ut streckkoder";
                this.cancelButton.Text = "Stäng";
                this.label1.Text = "Streckkod";
                ...
                this.label6.Text = "Leverantör";
                this.printGenerateButton.Text = "Skriv ut och generera XLS";
                this.printButton.Location = new System.Drawing.Point(586, 408);
                this.printButton.Size = new System.Drawing.Size(131, 23);
                this.printGenerateButton.Location = new System.Drawing.Point(405, 408);
        ...
        public List<printBarcodeItem> GetList(int orgID, int lokalID, int cabinetID, int orgAr, string nodeDB)
        {
            ...
            string AR_q = bc.GetAr(FormMain.Get.Databas, FormMain.Get.VerkId);
            ...
            {
                AR = Convert.ToInt32(item.Attributes["id"].Value);
            }
            ...
            try
            {
                inventoryList = bc.GetPrintingList(FormMain.Get.Databas, FormMain.Get.KemiDB, nodeDB, orgID, lokalID, cabinetID, orgAr,m_sRelease,sprak);
        ...
        private void getUser(string streckkod)
        {
            ...
            try
            {
                ...
                if (user == "")
                {
                    if (lang == true)
                    {
                        MessageBox.Show("Det här är inte en KLARA användare");
                    }
                    else
                    {
        ...
        public void ExportToExcel(List<printBarcodeItem> list)
        {
            System.Threading.Thread.CurrentThread.CurrentCulture = new System.Globalization.CultureInfo("en-US");
                ...
                if (lang == true)
                {
                    workSheet.Cells[1, "A"] = "UTSKRIVNA STRECKKODER";
                    if (Skap == "0" || Lada == "0")
                    {
                        workSheet.Cells[2, "A"] = "Streckkoder i " + Rum;
                    }
                    else
                    {
                        workSheet.Cells[2, "A"] = "Streckkoder i " + Rum + Separator + Skap + Separator + Lada;
                    }


                    workSheet.Cells[4, "A"] = "Streckkod";
                    workSheet.Cells[4, "B"] = "Produkt";
                    workSheet.Cells[4, "C"] = "Leverantör";
                    workSheet.Cells[4, "D"] = "CAS";
                    workSheet.Cells[4, "E"] = "Mängd";
                    workSheet.Cells[4, "F"] = "Notering";
                }
                else
                {
                ...
                if (lang == true)
                {
                    MessageBox.Show(string.Format("Filen '{0}' har sparats!", fileName));
                }
                else
                {
                ...
                catch (Exception exception)
                {
                    if (lang == true)
                    {
                        MessageBox.Show("Undantag", "Det uppstod ett PROBLEM när Excel-filen skulle sparas!\n" + exception.Message,MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    else
                    {
